...
MDM_PROXY_SECRET=<generated by ./gdlf init>
...
which adds the Caddy front-door container. `./gdlf init` generates
`MDM_PROXY_SECRET`; keep it the same in Caddy and rules-svc so direct
dashboard-port callers cannot spoof MDM client-certificate headers.
